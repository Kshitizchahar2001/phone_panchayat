// ignore_for_file: file_names, use_key_in_widget_constructors, unnecessary_this, prefer_if_null_operators, avoid_print, curly_braces_in_flow_control_structures

import 'package:flutter/material.dart';

class SwipeConfiguration {
  //Vertical swipe configuration options
  double verticalSwipeMaxWidthThreshold = 500.0;
  double verticalSwipeMinDisplacement = 10.0;
  double verticalSwipeMinVelocity = 0.5;
...
class SwipeDetector extends StatelessWidget {
  ...
  final Function(DragUpdateDetails dragUpdateDetails) onSwipeUpdate;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onVerticalDragEnd: (endDetails) {
        ...
        double calculatedVelocity = dy / timeElapsed;

        double calculatedPositiveVelocity =
            calculatedVelocity < 0 ? -calculatedVelocity : calculatedVelocity;
        print("calculated velocity is" + calculatedPositiveVelocity.toString());

        // double velocity = endDetails.primaryVelocity; // fix this

        //Convert values to be positive
        if (dx < 0) dx = -dx;
        if (dy < 0) dy = -dy;
        // double positiveVelocity = velocity < 0 ? -velocity : velocity;
        // print(positiveVelocity);
        ...
        if (calculatedPositiveVelocity <
            swipeConfiguration.verticalSwipeMinVelocity)
        ...
        if (calculatedVelocity < 0) {
          ...
          print("calculated velocity is " + calculatedVelocity.toString());
        }
      },
